...
export default class ListView extends Vue {
  ...
  @Prop({
    type: Function, default: (row: any, index: number) => {
      return true
    }
  ...
  created() {
    if (this.defaultSort) {
      this.internalDefaultSort = { prop: this.defaultSort.property, order: this.defaultSort.direction === 'ASC' ? 'ascending' : 'descending' }
